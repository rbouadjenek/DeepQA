...
def get_random_probabilities_list(num_of_arrays, array_size, num_of_positive_tokens):
	...
	for array in range(num_of_arrays):
		...
		random_start_index = helpers.get_random_numbers_between(array_size-num_of_positive_tokens) # the endindex is total length minus number of positive tokens
...
def main(num_of_positive_tokens):
	...
	for array_size in GV.ARRAY_SIZE_LIST:
		...
		for impurity in GV.IMPURITY_ARRAY:
			...
			milp_ef1_mean, milp_ef1_confidence_interval = mean_confidence_interval(milp_ef1_time_duration_list)
			brute_force_ef1_mean, brute_force_ef1_confidence_interval = mean_confidence_interval(brute_force_ef1_time_duration_list)
